...
describe('calculator engine — value correctness', () => {
  test('computes correct values for a known SaaS input', () => {
    const result = calculate({
      ...
      addbacks: 25_000,
      sliders: { growth: 4, owner_dep: 3, recurring: 4, cust_conc: 2, systems: 3, fin_records: 4 },
    })

    expect(result.adj_ebitda).toBe(575_000)
    expect(result.base_multiple).toBe(8.0)
    expect(result.estimated_multiple).toBe(10.3)
    expect(result.years_bonus).toBeCloseTo(0.3, 2)
    expect(result.revenue_scale_bonus).toBe(0.5)
    expect(result.valuation_base).toBe(5_922_500)
    expect(result.valuation_low).toBe(5_034_125)
    expect(result.valuation_high).toBe(6_810_875)
  ...
  test('computes correct values for a small restaurant', () => {
    const result = calculate({
      ...
      addbacks: 0,
      sliders: { growth: 2, owner_dep: 1, recurring: 2, cust_conc: 1, systems: 2, fin_records: 2 },
    })
